...
export async function incrementTotalLinesCompleted(audioVersionId: string) {
    ...
    try {
       ...
       .from("audio_screenplay_versions")
       .select("total_lines, total_lines_completed, screenplay_id")
       .eq("id", audioVersionId)
       .single();
     ...
     const { total_lines } = currentData;
      ...
      const { data: currentCount, error: fetchErrorVersions } = await supabase
        ...
        .eq("screenplay_id", currentData.screenplay_id) // Filter by screenplay_id

  
        logger.info('currentCount----------', { length: currentCount.length })
      ...
      logger.info('updatePayload', { updatedPayload: JSON.stringify(updatePayload) })
...
const processBatch = async (payload, tag, audioVersions, screenPlayVersionId) => {
...
export const trackAudioRuns = task({
  ...
  run: async (payload: unknown, { ctx }) => {
    ...
    try {
      ...
      // that can be setn to trigger.dev
      // is 100. We will do a smaller amount
      // to keep the queue filled
      let step = 50
      for (let i = 0; i < audioVersions.length; i += step) {
        await processBatch(payload, tag, audioVersions.slice(i,  i + step ), screenPlayVersionId)
      }
    } catch(e) {
      logger.error(e)
    }
